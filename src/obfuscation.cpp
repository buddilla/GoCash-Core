...
/* *** BEGIN OBFUSCATION MAGIC - gocash **********
    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat gocash and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 gocash per transaction on average.
            */
            if (r <= 10) {
...
bool CObfuscationPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizegocashAmount * COIN - pwalletMain->GetAnonymizedBalance();
...
void CObfuscationPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100gocash+1 ( bit on if present )
    // bit 1 - 10gocash+1
    // bit 2 - 1gocash+1
    // bit 3 - .1gocash+1
    // bit 3 - non-denom
...
void ThreadCheckObfuScationPool()
{
    ...
    RenameThread("gocash-obfuscation");
    ...
    while (true) {
        ...
        if (masternodeSync.IsBlockchainSynced()) {
            ...
            if (c % 60 == 0) {
                ...
                masternodGOCashments.CleanPaymentList();
