...
Value getgenerate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or gocash.conf setting gen)\n"
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "gocash is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "gocash is downloading blocks...");
...
//    result.push_back(Pair("sizelimit", (int64_t)MAX_BLOCK_SIZE));
    ...
    result.push_back(Pair("masternode_payments", pblock->nTime > Params().StartMasternodGOCashments()));
