...
#include "ui_zgoodscontroldialog.h"
...
std::list<std::string> ZGoodsControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZGoodsControlDialog::listMints;

ZGoodsControlDialog::ZGoodsControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZGoodsControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZGoodsControlDialog::~ZGoodsControlDialog()
{
    delete ui;
}

void ZGoodsControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZGoodsControlDialog::updateList()
{
...
void ZGoodsControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZGoodsControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZGoods_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZGoodsControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZGoodsControlDialog::GetSelectedMints()
{
...
void ZGoodsControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
