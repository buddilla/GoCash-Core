...
#include "ui_zgocashcontroldialog.h"
...
std::list<std::string> ZgocashControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZgocashControlDialog::listMints;

ZgocashControlDialog::ZgocashControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZgocashControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZgocashControlDialog::~ZgocashControlDialog()
{
    delete ui;
}

void ZgocashControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZgocashControlDialog::updateList()
{
...
void ZgocashControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZgocashControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZgocash_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZgocashControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZgocashControlDialog::GetSelectedMints()
{
...
void ZgocashControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
