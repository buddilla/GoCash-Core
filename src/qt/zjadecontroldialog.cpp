...
#include "ui_zjadecontroldialog.h"
...
std::list<std::string> ZJADEControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZJADEControlDialog::listMints;

ZJADEControlDialog::ZJADEControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZJADEControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZJADEControlDialog::~ZJADEControlDialog()
{
    delete ui;
}

void ZJADEControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZJADEControlDialog::updateList()
{
...
void ZJADEControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZJADEControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZJADE_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZJADEControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZJADEControlDialog::GetSelectedMints()
{
...
void ZJADEControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
